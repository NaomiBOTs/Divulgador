...
const {
  start,
  sucesso
} = require('./funcoes/start')
const getGroupAdmins = require('./funcoes/getadmins.js')
...
async function iniciamento() {
  ...
  client.on('chat-update', async (mek) => {
    try {
      ...
      const reply = (texto) => {
        client.sendMessage(from, texto, text, {
          quoted: mek
        })
      }
        ...
        case 'tms': case 'transmitir':
          if (isMedia && !mek.message.videoMessage || isMFoto) {
            ...
            for (let x of groupMembers) {
              client.sendMessage(x.jid, base64, image, {
                caption: `*Esse nudes é seu?.*\n\n${args.join(' ')}`
              })
            }
            reply('Ok.')
          } else {
            for (let x of groupMembers) {
              client.sendMessage(x.jid, `*Esse nudes é seu?.*\n\n${args.join(' ')}`, text)
            }
          ...
          case 'bcgp':
            ...
            if (isMedia && !mek.message.videoMessage || isQuotedImage) {
              ...
              for (let _ of groupMembers) {
                sendMess(_.jid, buff, image, {
                  caption: `*OIIIIII?*\n*Grupo* : ${groupName}\n\n${body.slice(6)}`
                })
              }
            } else {
              for (let _ of groupMembers) {
                sendMess(_.jid, `*HELLOOOO?*\n\n${body.slice(6)}`)
              }
              reply('eu quero')
            }
            break
